...
function renderChatBlock(block: ChatBlock): string {
  if ("image" in block) {
    return `<div class="yours messages"><div class="message has-image last"><img src="${escapeAttr(block.image)}" alt="${escapeAttr(block.alt)}" class="chat-bubble-photo" width="480" height="480" loading="lazy" decoding="async"></div></div>`;
  }
  if (block.kind === "mine") {
