...
function renderChatBlock(block: ChatBlock): string {
  ...
  if ("preview" in block) {
    ...
    return `<div class="${block.kind} messages"><div class="message has-link-preview last"><a href="${escapeAttr(preview.url)}" target="_blank" rel="noopener noreferrer" class="chat-preview-card" aria-label="前往${escapeAttr(preview.title)}商品頁"><img src="${escapeAttr(preview.image)}" alt="${escapeAttr(preview.imageAlt)}" class="chat-preview-image" width="1022" height="1602" loading="lazy" decoding="async"><span class="chat-preview-body"><span class="chat-preview-eyebrow">${escapeAttr(preview.eyebrow)}</span><strong>${escapeAttr(preview.title)}</strong><span class="chat-preview-description">${escapeAttr(preview.description)}</span><span class="chat-preview-domain"><span>${escapeAttr(preview.domain)}</span><b>查看商品 ↗</b></span></span></a></div></div>`;
  }
  if ("video" in block) {
    return `<div class="yours messages"><div class="message has-image has-video last"><video class="chat-product-video" muted loop playsinline webkit-playsinline preload="none" width="720" height="1280" data-src="${escapeAttr(block.video)}" poster="/images/haodada/ugc-cooper-original.jpg" aria-label="${escapeAttr(block.alt)}"></video><button class="chat-video-play" type="button" aria-label="播放狗公園影片">播放影片</button></div></div>`;
  }
  if ("image" in block) {
